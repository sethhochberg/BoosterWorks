...
class Auth extends CI_Controller
{
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$data['view']['do'] = "Please login with your primary email address and password.";
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('password'),
						$email_activation))) {									// success creating user, so now add profile
						...
						{
							$this->session->set_flashdata('notice', '<div class="success">Thanks for registering, ' . set_value('first_name') . '! Use the form below to login and begin using the system.</div>');
							
							redirect('auth/login');   //write okay, let them log in
						}
						else
						{
			...
			$data['form_open'] = form_open('auth/register');
